    ...
    void Start()
    {
        ...
        for(int i=0; i<Altezza; i++)
            for(int j=0; j<Larghezza; j++){
                if((i+j)%2==0)
                Tile = Instantiate(FloorTile, new Vector3(i,0,j), Quaternion.identity);
                else
                Tile = Instantiate(FloorTile1, new Vector3(i,0,j), Quaternion.identity);
                Tile.transform.parent=gameObject.transform;
            }   
        //Muri laterali
        for(int i=0; i<Altezza; i++){
            for(int j=0; j<BordoOrizzontale; j++){
                Tile = Instantiate(Terra, new Vector3(i,1,j), Quaternion.identity);
                Tile.transform.parent=gameObject.transform;
            }
